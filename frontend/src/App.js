...
const tokenValido = (t) => {
  try {
    const p = parseJwt(t);
    if (!p) return false;
    if (p.exp) return p.exp * 1000 > Date.now();
    return true;
  } catch { return false; }
};
...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  ::-webkit-scrollbar-thumb{background:${G.cardBorder};border-radius:3px}
  ...
  .btn-primary{background:${G.accent};color:#0a1520}
  .btn-primary:hover{background:${G.accentDim}}
  .btn-ghost{background:transparent;border:1px solid ${G.cardBorder};color:${G.textMuted}}
  .btn-ghost:hover{border-color:${G.accent};color:${G.accent}}
  .btn-danger{background:transparent;border:1px solid transparent;color:${G.danger}}
  .btn-danger:hover{background:rgba(255,77,109,0.1)}
  .btn-logout{background:transparent;border:1px solid rgba(255,77,109,0.3);color:${G.danger};font-size:11px;padding:6px 12px}
  .btn-logout:hover{background:rgba(255,77,109,0.1);border-color:${G.danger}}
  ...
  .kpi-grid-7{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:24px}
  ...
  .kpi-card.green::before{background:${G.accent}}
  .kpi-card.red::before{background:${G.danger}}
  .kpi-card.orange::before{background:${G.warning}}
  ...
  .kpi-card.red .kpi-value{color:${G.danger}}
  .kpi-card.orange .kpi-value{color:${G.warning}}
  ...
  .chart-grid-3{display:grid;grid-template-columns:2fr 1fr 1fr;gap:20px;margin-bottom:24px}
  ...
  .modal{background:${G.card};border:1px solid ${G.cardBorder};border-radius:8px;width:540px;max-width:95vw;overflow:hidden;max-height:90vh;overflow-y:auto}
  ...
  .login-wrap{min-height:100vh;background:${G.bg};display:flex;align-items:center;justify-content:center;background-image:radial-gradient(ellipse at 20% 50%,rgba(0,229,160,0.04) 0%,transparent 60%),radial-gradient(ellipse at 80% 20%,rgba(77,166,255,0.04) 0%,transparent 50%)}
        ...
        <div className="modal-body">
          <div className="form-grid-2">
            ...
            <div className="field"><label>Fecha *</label>
              <input type="date" value={form.fecha_programada} onChange={e=>setForm({...form,fecha_programada:e.target.value})} /></div>
          ...
          <div className="field"><label>Descripción</label>
            <textarea value={form.descripcion} onChange={e=>setForm({...form,descripcion:e.target.value})} placeholder="Actividades a realizar..." /></div>
        </div>
      ...
      <div className="modal">
        ...
        <div className="modal-body">
          ...
          <div className="field"><label>Observaciones</label>
            <textarea value={obs} onChange={e=>setObs(e.target.value)} placeholder="Describe lo realizado, repuestos usados..." /></div>
        </div>
        <div className="modal-footer">
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
      ...
      <div className="modal">
        ...
        <div className="modal-body">
          <div className="form-grid-2">
            ...
            <div className="field"><label>Tipo de reporte</label>
              <select value={form.tipo} onChange={e=>setForm({...form,tipo:e.target.value})}>
                {['Incidente','Evento Adverso','Alerta de Seguridad','Falla Técnica'].map(t=><option key={t}>{t}</option>)}
              </select></div>
            <div className="field"><label>Fecha del evento *</label>
              <input type="date" value={form.fecha_evento} onChange={e=>setForm({...form,fecha_evento:e.target.value})} /></div>
          ...
          <div className="field"><label>Descripción del evento *</label>
            <textarea value={form.descripcion} onChange={e=>setForm({...form,descripcion:e.target.value})} placeholder="Describe detalladamente el evento o incidente..." style={{minHeight:100}} /></div>
        </div>
        <div className="modal-footer">
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
...
function ModalUsuario({ usuario, token, onClose, onSaved }) {
      ...
      <div className="modal">
        ...
        <div className="modal-body">
          <div className="form-grid-2">
            ...
            <div className="field"><label>Correo electrónico *</label><input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} /></div>
            <div className="field"><label>Rol</label>
              <select value={form.rol} onChange={e=>setForm({...form,rol:e.target.value})}>
                {['Admin','Biomedico','Ingeniero','Auditor'].map(r=><option key={r}>{r}</option>)}
              </select></div>
            <div className="field"><label>{esNuevo?'Contraseña *':'Nueva contraseña (opcional)'}</label>
              <input type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} placeholder={esNuevo?'Mínimo 6 caracteres':'Dejar vacío para no cambiar'} /></div>
          </div>
        </div>
        <div className="modal-footer">
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
          <button className="btn btn-primary" onClick={guardar} disabled={saving}>{saving?'Guardando...':(esNuevo?'+ Crear usuario':'✓ Guardar cambios')}</button>
        </div>
      </div>
    </div>
  );
...
function App() {
  ...
  const logout = () => { localStorage.removeItem('biomed_token'); setTokenState(null); };
  ...
  const alertas = mantenimientos.filter(m=>{
    const diff = (new Date(m.fecha_programada)-new Date())/(1000*60*60*24);
    return m.estado==='PENDIENTE' && diff<=7;
  });

  const filtrados   = equipos.filter(e=>
    e.nombre?.toLowerCase().includes(filtro.toLowerCase())||
    e.marca?.toLowerCase().includes(filtro.toLowerCase())||
    e.serie?.toLowerCase().includes(filtro.toLowerCase())
  );
  const mantFiltrados = filtroMant==='TODOS' ? mantenimientos : mantenimientos.filter(m=>m.estado===filtroMant);

  if (!token) return <LoginPro setToken={setToken} />;
  ...
  const navItems = [
    ...
    ...(rol==='Admin' ? [{ id:'usuarios', icon:'◉', label:'Usuarios' }] : []),
  ];
  ...
  return (
    <>
      ...
      {modalUsuario !== null && (
        <ModalUsuario usuario={modalUsuario||null} token={token}
          onClose={()=>setModalUsuario(null)} onSaved={()=>{cargarTodo();setModalUsuario(null);}} />
      )}
      ...
      <div className="layout">
        ...
        <aside className="sidebar">
            ...
            {navItems.map(n=>(
              <div key={n.id} className={`nav-item ${seccion===n.id?'active':''}`} onClick={()=>setSeccion(n.id)}>
                ...
                {n.id==='mantenimiento' && alertas.length>0 && (
                  <span style={{marginLeft:'auto',background:G.danger,color:'#fff',borderRadius:'10px',padding:'1px 6px',fontSize:10,fontWeight:700}}>
                    {alertas.length}
                  </span>
                )}
                {n.id==='tecnovigilancia' && tecno.filter(t=>t.estado==='ABIERTO').length>0 && (
                  <span style={{marginLeft:'auto',background:'#f4a261',color:'#fff',borderRadius:'10px',padding:'1px 6px',fontSize:10,fontWeight:700}}>
                    {tecno.filter(t=>t.estado==='ABIERTO').length}
                  </span>
                )}
              </div>
            ))}
          ...
          <div className="sidebar-footer">
            ...
            <div className="rol-badge">{rol}</div>
            <button className="btn btn-logout" style={{marginTop:12,width:'100%',justifyContent:'center'}} onClick={logout}>
              ↩ Cerrar sesión
            </button>
          </div>
        </aside>
        ...
        <main className="main">
          ...
          <div className="topbar">
            ...
            <div className="topbar-right">
              <button className="btn btn-ghost" onClick={descargarPDF}>↓ Reporte PDF</button>
              ...
              {rol==='Admin'   && seccion==='usuarios'       && <button className="btn btn-primary" onClick={()=>setModalUsuario(false)}>+ Nuevo usuario</button>}
            </div>
          </div>
          ...
          <div className="content">
            ...
            {seccion==='dashboard' && (
              <>
                ...
                <div className="chart-grid">
                  ...
                  <div className="panel">
                    <div className="panel-header"><div className="panel-title">Equipos por Servicio Clínico</div></div>
                    <div className="panel-body" style={{height:260}}>
                      ...
                      ) : <div className="empty-state">Sin datos de servicios</div>}
                    </div>
                  </div>
                  ...
                  <div className="panel">
                    ...
                    <div className="panel-body" style={{height:260}}>
                      ...
                      ) : <div className="empty-state">Sin datos de mantenimiento</div>}
                    </div>
                  </div>
                </div>
                ...
                <div className="chart-grid">
                  ...
                  <div className="panel">
                    <div className="panel-header"><div className="panel-title">Equipos por Clasificación de Riesgo</div></div>
                    <div className="panel-body" style={{height:240}}>
                      {dashKpis?.porRiesgo?.length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie data={dashKpis.porRiesgo} dataKey="total" nameKey="riesgo"
                              cx="50%" cy="50%" outerRadius={80} label={({riesgo,total})=>`${riesgo}: ${total}`}
                              labelLine={{stroke:G.textMuted}}>
                              {dashKpis.porRiesgo.map((_,i)=>(
                      ...
                      ) : <div className="empty-state">Sin datos de riesgo</div>}
                    </div>
                  </div>
                  ...
                  <div className="panel">
                    ...
                    <div className="panel-body" style={{height:240}}>
                      {dashKpis?.porGravedad?.length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie data={dashKpis.porGravedad} dataKey="total" nameKey="gravedad"
                              cx="50%" cy="50%" outerRadius={80} label={({gravedad,total})=>`${gravedad}: ${total}`}>
                              {dashKpis.porGravedad.map((e,i)=>(
                                <Cell key={i} fill={e.gravedad==='GRAVE'?G.danger:e.gravedad==='MODERADO'?G.warning:G.textMuted} />
                              ))}
                            </Pie>
                            <Tooltip contentStyle={{background:G.card,border:`1px solid ${G.cardBorder}`,borderRadius:4,fontSize:12}} />
                          </PieChart>
                        </ResponsiveContainer>
                      ) : <div className="empty-state">Sin reportes de tecnovigilancia</div>}
                    </div>
                  </div>
                </div>
              </>
            ...
            {seccion==='inventario' && (
              <>
                <div className="kpi-grid">
                  {[
                    {label:'Total equipos', valor:total,     cls:'blue'},
                    {label:'Activos',       valor:activos,   cls:'green'},
                    {label:'Por vencer',    valor:porVencer, cls:'orange'},
                    {label:'INVIMA vencido',valor:vencidos,  cls:'red'},
                  ].map(k=>(
                    <div key={k.label} className={`kpi-card ${k.cls}`}>
                      <div className="kpi-label">{k.label}</div>
                      <div className="kpi-value">{k.valor}</div>
                ...
                {alertas.length>0 && (
                  <div className="alert-bar">
                    ...
                    <div className="alert-list">
                      <div className="alert-title">Mantenimientos críticos ({alertas.length})</div>
                      {alertas.map(a=><div key={a.id} className="alert-item">{a.equipo_nombre} — vence {formatFecha(a.fecha_programada)}</div>)}
                    </div>
                  </div>
                )}
                  ...
                  <div className="panel">
                    ...
                    <div className="panel-body">
                      <div className="form-grid">
                        {[['nombre','Nombre del equipo','text'],['marca','Marca','text'],['modelo','Modelo','text'],
                          ['serie','N° de serie','text'],['registro_invima','Registro INVIMA','text'],
                          ['fecha_vencimiento_invima','Vencimiento INVIMA','date'],
                          ['ubicacion','Ubicación','text'],['servicio','Servicio clínico','text']
                        ].map(([key,label,type])=>(
                        ...
                        <div className="field"><label>Clasificación riesgo</label>
                      ...
                      <button className="btn btn-primary" onClick={guardar}>{editando?'✓ Guardar cambios':'+ Registrar equipo'}</button>
                    </div>
                  </div>
                )}
                ...
                <div className="search-bar">
                  <span>⌕</span>
                  <input placeholder="Buscar por nombre, marca o serie..." value={filtro} onChange={e=>setFiltro(e.target.value)} />
                </div>

                <div className="panel">
                  <div className="panel-header">
                    <div className="panel-title">Equipos registrados</div>
                    <div style={{fontSize:11,color:G.textMuted}}>{filtrados.length} resultado{filtrados.length!==1?'s':''}</div>
                  </div>
                  {filtrados.length===0 ? <div className="empty-state">No hay equipos registrados</div> : (
                        ...
                        {filtrados.map(eq=>{
                          ...
                          return (
                            <tr key={eq.id}>
                              ...
                              <td>
                                <div style={{fontSize:11,fontFamily:'IBM Plex Mono',color:G.textMuted}}>{formatFecha(eq.fecha_vencimiento_invima)}</div>
                                {inv&&<span className={`badge ${inv.cls}`} style={{marginTop:4,display:'inline-block'}}>{inv.label}</span>}
                              </td>
            ...
            {seccion==='mantenimiento' && (
              <>
                <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:16,marginBottom:24}}>
                  {[
                    {label:'Total OTs',   valor:kpis?.total??'—',      cls:'blue'},
                    {label:'Pendientes',  valor:kpis?.pendientes??'—',  cls:'orange'},
                    {label:'Realizados',  valor:kpis?.realizados??'—',  cls:'green'},
                    {label:'Críticas',    valor:kpis?.criticas??'—',    cls:'red'},
                    {label:'MTTR (días)', valor:kpis?.mttr??'—',        cls:'purple', sub:'Tiempo medio reparación'},
                  ].map(k=>(
                    <div key={k.label} className={`kpi-card ${k.cls}`}>
                      <div className="kpi-label">{k.label}</div>
                      <div className="kpi-value">{k.valor}</div>
                ...
                {alertas.length>0&&(
                  <div className="alert-bar">
                    <span className="alert-icon">⚠</span>
                    <div className="alert-list">
                      <div className="alert-title">OTs que vencen en ≤7 días ({alertas.length})</div>
                      {alertas.map(a=><div key={a.id} className="alert-item">{a.equipo_nombre} · {a.tipo} — {formatFecha(a.fecha_programada)}</div>)}
                    </div>
                  </div>
                )}
                ...
                <div className="panel">
                  <div className="panel-header">
                    <div className="panel-title">Órdenes de trabajo</div>
                    <span className="badge badge-gray">{mantFiltrados.length} registros</span>
                  </div>
                  {mantFiltrados.length===0 ? (
                    <div className="empty-state">No hay órdenes de trabajo.{rol!=='Auditor'?' Crea una con "+ Nueva OT"':''}</div>
                  ) : (
                    <table className="data-table">
                      <thead><tr>
                        <th>Equipo</th><th>Servicio</th><th>Tipo</th>
                        <th>Prioridad</th><th>Fecha prog.</th><th>Fecha real.</th><th>Estado</th>
                        {rol!=='Auditor'&&<th></th>}
                      </tr></thead>
                      <tbody>
                        {mantFiltrados.map(m=>(
                          <tr key={m.id}>
                            ...
                            {rol!=='Auditor'&&(
                              <td>
                                {m.estado==='PENDIENTE'&&<button className="btn btn-primary btn-icon" onClick={()=>setModalFin(m)}>✓ Finalizar</button>}
                                {m.estado==='REALIZADO'&&m.observaciones&&<span style={{fontSize:11,color:G.textMuted}} title={m.observaciones}>📋</span>}
                              </td>
                            )}
                          </tr>
                        ))}
            ...
            {seccion==='tecnovigilancia' && (
              <>
                <div className="kpi-grid">
                  {[
                    {label:'Total reportes', valor:tecno.length,                               cls:'blue'},
                    {label:'Abiertos',       valor:tecno.filter(t=>t.estado==='ABIERTO').length, cls:'red'},
                    {label:'En revisión',    valor:tecno.filter(t=>t.estado==='EN_REVISION').length, cls:'orange'},
                    {label:'Cerrados',       valor:tecno.filter(t=>t.estado==='CERRADO').length, cls:'green'},
                  ].map(k=>(
                    <div key={k.label} className={`kpi-card ${k.cls}`}>
                      <div className="kpi-label">{k.label}</div>
                      <div className="kpi-value">{k.valor}</div>
                ...
                <div className="panel">
                  <div className="panel-header">
                    <div className="panel-title">Reportes de Tecnovigilancia</div>
                    <span className="badge badge-gray">{tecno.length} registros</span>
                  </div>
                  {tecno.length===0 ? (
                    <div className="empty-state">No hay reportes.{rol!=='Auditor'?' Crea uno con "+ Nuevo reporte"':''}</div>
                  ) : (
                    <table className="data-table">
                      <thead><tr>
                        <th>Fecha</th><th>Equipo</th><th>Tipo</th>
                        <th>Gravedad</th><th>Estado</th><th>Reportado por</th><th>Descripción</th>
            ...
            {seccion==='historial' && (
              <div className="panel">
                <div className="panel-header">
                  <div className="panel-title">{equipoSel?`Trazabilidad — ${equipoSel.nombre}`:'Selecciona un equipo desde Inventario'}</div>
                </div>
                <div className="panel-body">
                  {!equipoSel ? (
                    <div className="empty-state">Ve a Inventario y presiona ◷ en cualquier equipo</div>
                  ) : historial.length===0 ? (
            ...
            {/* ═══ USUARIOS (solo Admin) ═══ */}
            {seccion==='usuarios' && rol==='Admin' && (
              <>
                <div className="kpi-grid">
                  {[
                    {label:'Total usuarios', valor:usuarios.length,                                 cls:'blue'},
                    {label:'Admins',         valor:usuarios.filter(u=>u.rol==='Admin').length,      cls:'purple'},
                    {label:'Biomédicos',     valor:usuarios.filter(u=>u.rol==='Biomedico').length,  cls:'green'},
                    {label:'Auditores',      valor:usuarios.filter(u=>u.rol==='Auditor').length,    cls:'gray'},
                  ].map(k=>(
                    <div key={k.label} className={`kpi-card ${k.cls||'blue'}`}>
                      <div className="kpi-label">{k.label}</div>
                      <div className="kpi-value">{k.valor}</div>
                    </div>
                  ))}
                ...
                <div className="panel">
                  <div className="panel-header">
                    ...
                    <span className="badge badge-gray">{usuarios.length} usuarios</span>
                  </div>
                      ...
                      <tbody>
                        {usuarios.map(u=>(
                          <tr key={u.id}>
                            ...
                            <td>
                              <span className={`badge ${u.rol==='Admin'?'badge-purple':u.rol==='Auditor'?'badge-gray':'badge-green'}`}>
                                {u.rol}
                              </span>
                            </td>
